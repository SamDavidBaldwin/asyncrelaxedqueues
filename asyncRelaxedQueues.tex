...
We can now prove the invariants of our labeling scheme that enable us to argue that all $Dequeue$ instances return legal values.
...
\begin{lemma}
  When an element $x$ is labeled $p_i$, no other process will return $x$ to a $Dequeue$ it invoked.
\end{lemma}
...
\begin{proof}
  We proceed by induction on $\pi$.  The empty sequence is legal.  We assume that for an arbitrary prefix $\rho \cdot op$ of $\pi$, $\rho$ is legal and will show that $\rho \cdot op$ is also legal.  Let $p_i$ be $op$'s invoking process and proceed by cases on $op$'s operation type:
  \begin{itemize}
  \item $op$ is an $Enqueue$ instance.  Since $Enqueue$ has no return value, $\rho \cdot op$ is always legal.
  \item $op$ is a slow $Dequeue$ instance, $op = Dequeue(-,ret)$.  Assume first that $ret \neq \bot$.  $p_i$ chose $ret$ from its $lQueue$ in either line~\ref{oooline:sDeqChooseLabeled} or \ref{oooline:sDeqChooseUnlabeled}.  
  \item $op$ is a fast $Dequeue$ instance.
  \end{itemize}
\end{proof}

  %In the case where $k > n$, since the number of messages required to have a successful dequeue in bounds is less
