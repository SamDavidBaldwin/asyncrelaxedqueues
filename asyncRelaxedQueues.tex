...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    \Function{Enqueue}{$val$}
      ...
      \State $Enq_{ts} = v_i$ \Comment{\red{For compactness, maybe have incrementTS() return the new vector, so we can combine with previous line?}}
      \State send $(EnqReq, val, i, Enq_{ts})$ to all processes \Comment{\red{Switch $i$, $End_{ts}$ for consistency with Dequeue}}
    \EndFunction
    ...
    \Function{Receive}{$(safetyFlag, Deq_{ts}, i, p_{invoker})$ from $p_j$}
      ...
      \If{$confirmationList.ts == Deq_{ts}$}
        ...
        \If{$confirmationList.full()$ and not $confirmationList.handled$}
          ...
          \If{$p_i == p_{invoker}$} \Return $ret$ to user \EndIf \Comment{\red{I think this shouldn't be $p_{invoker}$ from the message receipt, this should be the invoking process of the dequeue instance from confLists we're locally executing.}}
        \EndIf 
      \EndFor
      \EndFunction
      \Statex
