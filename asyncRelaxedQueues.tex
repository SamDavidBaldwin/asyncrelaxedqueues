...
Distributed data structures are a fundamental construct for computing to spread across different machines which need to work together.  By specifying the interface which dta operations may use and the exact effects of those operations, we provide an abstraction layer that allows a distributed programmer to not worry about the details of coordinating and maintaining data, instead focusing on the particular application in which they are interested.  We, in building that abstraction layer, must consider all the possible oddities of concurrency, messages, delays, and different views.  We can thus focus on optimizing our algorithm, and provide the developer with an efficient tool which provides well-specified guarantees.

We consider the problem of building such distributed data structures in such a way as to minimize the delay users experience.  Past work has given tight or nearly tight bounds on the possible implementations of a variety of data structures \cite{Kosa99,WangTalmageLeeWelch18} in partially synchronous systems, were there are bounds on the real time which messages between participating processes may take to arrive and processes know and can depend on those bounds.  However, real-world systems do not generally have reliable bounds on timings, so these algorithms are difficult to implement
