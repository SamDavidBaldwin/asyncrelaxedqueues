...
% Specifically: lQueue.deqByIndex(pos,ts) does not return a value with later timestamp than ts, instead returning the element at index pos in the sublist of elements with timestamp preceding ts, or \bot if none such exists.
...
%      \State $propagateEarlierResponses(PendingDequeues,j)$ \Comment{\red{Cole/Sam: Do we need this?  Is it an optimization?  I feel like it complicates the proof, since I need to argue that the earlier $Dequeue$ instance is safe to locally execute. (I think true, but what is the benefit?)}}
...
%        \If{$confirmationList.full()$}\label{fifoline:fullConfList}
      \If{all $response$s in $confirmationList$ are $True$}\label{fifoline:fullConfList}
        \State $ret = lQueue.deqByIndex(index, Deq_{ts})$\label{fifoline:chooseDeqValue} \Comment{Locally execute the $Dequeue$ instance, leaving elements for prior pending instances} 
...
%    \For{$row$ from $len(PendingDequeues) - 1$ down to $1$} \Comment{If a later-timestamp instance receives a $DeqAck$ from $p_j$, then any earlier-timestamp processes do not need to wait for a $DeqAck$ from $p_j$.} 
...
%      \If{$response$ is $True$ and $PendingDequeues[row-1].responses[j]$ is $False$}
%        \State $PendingDequeues[row-1].responses[j] = True$
%      \Else\ break
%      \EndIf
%    \EndFor
%    \EndFunction
...
%\begin{proof}
%  We proceed by induction on the order of $Dequeue$ instances in $\pi$.  

%  It is worth noting first that every process will locally execute every $Dequeue$ instance since processes send $DeqAck$ to all on line~\ref{fifoline:sendDeqAck}, and our assumption of reliable channels means that all processes will receive those $DeqAck$ messages and eventually pass the check in line~\ref{fifoline:fullConfList} and locally execute the instance.

%  For our base case, when $\pi$ is empty, then the claim that all processes delete the same values for each $Dequeue$ instance in $\pi$ is vacuously true.  We then assume for an arbitrary prefix $\rho$ of $\pi$, all processes delete the same value for every instance in $\pi$.

%  For the inductive step, consider when some process $p_i$ locally executes $Dequeue$ instance $op$ which appears in $\pi$ following the prefix $\rho$.  Let $x$ be the value $p_0$ deletes from its $lQueue$ on line~\ref{fifoline:chooseDeqValue} for $op$.  By Lemma~\ref{fifolem:prevLocalExec}, when any other process $p_j$ locally executes $op$, it sees the same history of lower-timestamp instances that $p_0$ does.  The difference is that 

%  Since the choice of which value to delete is a deterministic function of these sequences, there are only two possible reasons $p_j$ could choose a different value.  First, if some $Dequeue$ instance is in $p_0$'s $PendingDequeues$ but $p_j$ has already locally executed it (or vice versa), or second, if $p_j$ has already locally executed some $Dequeue$ instance with larger timestamp than $op$.

%  For the first case, suppose that $p_j$ has already locally executed some $Dequeue$ instance $op'$ with $ts(op') < ts(op)$ that is still in $p_0$'s $PendingDequeues$.  When $p_0$ locally executes $op$, since $op'$ has a smaller timestamp, $p_0$ leaves an element in its $lQueue$ with smaller timestamp than that it returns for $op$.  This has the same effect as $p_j$'s local execution, when $op'$ has already deleted an element 

  % if p_j already locally executed a dequeue p_0 hasn't yet, argue that that execution left x
  % 
  
%  By Lemma~\ref{fifolem:prevLocalExec}, the only relevant differences in local state when two processes $p_i$ and $p_j$ locally execute any $Dequeue$ instance $op$ are whether the processes have locally executed earlier-timestamp $Dequeue$ instances or are still holding them in $PendingDequeues$, waiting for confirmations.  It is possible that $p_i$ and $p_j$ have locally executed different sets of instances with later timestamps, but those differences will not affect the value deleted when executing $op$, since line~\ref{fifoline:chooseDeqValue} ignores the arguments of $Enqueue$ instances with lexicographically-later timestamps and, as we will prove here, later-timestamped $Dequeue$ instances will leave the correct element for $op$ in the local replica.

%  Suppose that when $p_i$ locally executes $op$, it deletes value $val$ from its $lQueue$.  Let $x$ be the number of $Enqueue$ instances and $y$ be the number of $Dequeue$ instances in $\pi$ with lexicographically-smaller timestamps than $op$.  
%\end{proof}
...
\begin{proof}
  ...
  % Note: Don't need this paragraph, as "oldest unmatched" below covers it--can't double dequeue because the argument is no longer unmatched.
...
%
% Note: we proved legality without this, so I don't think it's needed.
%\begin{lemma}
...
%      \State $propagateEarlierResponses(PendingDequeues)$ 
      \State Define $currentConfList$ a sthe confirmation list in $PendingDequeues$ which has $ts == Deq_{ts}$
          \State $confirmationList.responses[j] = safetyFlag$
      \For{$(index, confirmationList)$ in $PendingDequeues$}
        \If{all $responses$ in $confirmationList$ are $True$}
          \If {$op == Deq_f$ and $p_i \neq p_{inv}$}
