...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    \Function{Enqueue}{$val$}
      \State $EnqCount = 0$ \Comment{\red{What is this for? Is it a response counter?  Maybe rename.}}
      \State $incrementTS(v_i)$ \Comment{\red{Is there any reason to pass my own timestamp, instead of just relying on having access to it in the helper?}}
      \State $Enq_{ts} = v_i$ \label{fifoline:enqTS}\Comment{\red{For compactness, maybe have incrementTS() return the new vector, so we can combine with previous line?}}
      \State send $(EnqReq, val, i, Enq_{ts})$ to all processes \label{fifoline:sendEnqReq} \Comment{\red{Switch $i$, $End_{ts}$ for consistency with Dequeue}}
    \EndFunction

    \Function{Receive}{$EnqReq, val, j, Enq_{ts}$} from $p_j$
      \State $updateTS(v_i, Enq_{ts})$ \label{fifoline:enqReqTSUpdate}
      \State $lQueue.insertByTS$($val, j, Enq_{ts}$)\label{fifoline:executeEnq}
      \For{$confirmationList$ in $PendingDequeues$}
        \If {$confirmationList.ts \prec Enq_{ts}$}
          \State $confirmationList.responses[j] = Safe$ \Comment{\red{Please confirm that this is still valid, even with broadcast safes.}}
        \EndIf
      \EndFor

      \State send $(EnqAck, i)$ to $p_j$ \label{fifoline:sendEnqAck}
    \EndFunction
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \Function{Dequeue}{}
      \State $incrementTS(v_i)$ \Comment{\red{Same questions as in Enqueue.}}
      \State let $Deq_{ts} = v_i$\label{fifoline:deqTS}
      \State send $(DeqReq, Deq_{ts}, i)$ to all processes \label{fifoline:sendDeqReq}
    \EndFunction

    \Function{Receive}{$(DeqReq, Deq_{ts}, j)$ from $p_j$}
    \State $updateTS(Deq_{ts})$ \label{fifoline:deqReqTSUpdate} \Comment{\red{This was missing, but seems important.}}
    \State let $p_{invoker} = p_j$ \Comment{\red{We're just renaming, right?  How about we just name it $p_{inv}$ or similar in the function header?}}
      \State let $Deq_{ts} = v_i$ \Comment{\red{This doesn't look right to me.  The $Deq_{ts}$ is what we got in the message, right? I think we can delete this line.}}
      \If{$Deq_{ts}$ is not in $PendingDequeues$}
      \State $PendingDequeues.insertByTs(createList(Deq_{ts}, p_{invoker}$))\label{fifoline:savePendingDeq}
      \EndIf
      \If{$Deq_{ts} \neq 0$ and $Deq_{ts} \prec v_i$} \Comment{\red{We should be comparing the recieved ts ($Deq_{ts}$) with the ts of our own pending dequeue, which we don't seem to be storing anywhere.}}
        \State let $safetyFlag = Unsafe$
      \Else
        \State let $safetyFlag = Safe$
      \EndIf
      \State send $(safetyFlag, Deq_{ts}, i, p_{invoker})$ to all processes \label{fifoline:sendSafetyFlag}\label{fifoline:sendDeqAck}
    \EndFunction
    ...
    \Function{Receive}{$(safetyFlag, Deq_{ts}, i, p_{invoker})$ from $p_j$}
      ...
      \If{$confirmationList.ts == Deq_{ts}$}
        ...
        \EndIf 
        \If{$confirmationList.full()$ and not $confirmationList.handled$}\label{fifoline:fullConfList}
          ...
          \State let $ret = lQueue.deqByIndex(pos)$\label{fifoline:chooseDeqValue}
          \State $updateUnsafes(PendingDequeues, index)$
          \If{$p_i == p_{invoker}$} \Return $ret$ to user \label{fifoline:deqReturn} \EndIf \Comment{\red{I think this shouldn't be $p_{invoker}$ from the message receipt, this should be the invoking process of the dequeue instance from confLists we're locally executing.}}
        \EndIf 
      \EndFor
      \EndFunction
      \Statex
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \For{$row$ from $len(PendingDequeues) - 1$ down to $1$} \Comment{\red{May need a more descriptive way to talk about iteration order.  These are sorted by timestamp, so are we moving in order of increasing or decreasing timestamp?}}
    \For{$col$ from $0$ to $n$} \Comment{\red{n-1?}}
    \State let $response = PendingDequeues[row].responses[col]$ 
    \If{$response \neq 0$ and $PendingDequeues[row-1].responses[col] == 0$} \Comment{\red{TODO: Update to use $safe$/$unsafe$/empty}}
    ...
    \State $ConfirmationList.responses = [0,0,...,0]$ \Comment{A list of $n$ 0s} \Comment{\red{Is $ConfirmationList$ defined?  Because it looks like we're setting attributes of an object we haven't initialized.  I get that this is basically a constructor, but maybe we can make this more straightforward?}}
